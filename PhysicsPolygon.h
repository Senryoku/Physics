...
#include "PhysicsCollidingObject.h"
 ...
 *
 **/
class Polygon : public CollidingObject
{
	protected:
...
class CollisionInfo {
	public:
		...
		CollisionInfo() : P1(0), P2(0), Depth(0), Normal(0,0), Edge(0), V(0)
		{
		}

		CollisionInfo(Polygon* P1, Polygon* P2, float D,
						Vec2 N, Rigid* R, Vertex* V) :
			P1(P1), P2(P2), Depth(D), Normal(N), Edge(R), V(V)
		{
		}
};
